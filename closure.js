...
//   return (input) => {
//     if (input === "undo") {
//       if (storage.length === 0) return "nothing to undo";
//       indicator -= 1;
//       return `${storage.pop()} undone`;
//     } else if (indicator + 1 > limit) {
//       storage.shift();
//       storage.push(input);
//       return `${storage[indicator - 1]} done`;
//     } else {
//       return `${(storage[indicator++] = input)} done`;
//     }
//   };
// }
...
function makeHistory(limit) {
  ...
  return (input) => {
    if (input === "undo") {
      ...
      if (limit === 0) return `${input} done`;
      storage.shift();
      storage.push(input);
      return `${storage[indicator]} done`;
    } else return `${(storage[++indicator] = input)} done`;
  };
}
...
function blackjack(array) {
  ...
  return (n1, n2) => {
    let startingIndex = arrayCurrentIndex;
    let isBust = false;
    return () => {
      const sum =
        n1 +
        n2 +
        array
          .slice(startingIndex, arrayCurrentIndex)
          .reduce((sum, num) => sum + num, 0);
      return isBust
        ? "you are done!"
        : sum <= 21
        ? (arrayCurrentIndex++, sum)
        : ((isBust = true), "bust");
    };
  };
}
